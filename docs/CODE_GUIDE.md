...
홈 공터 이미지는 `loadForestArt()`가 숲 배경, 다섯 종류별 기본 가구와 상점 상품별 실제 가구 이미지를
카탈로그 ID로 불러와 장면에 주입한다. 배치 데이터에 `shopItemId`가 있으면 상품별 이미지를 사용하고,
이전 저장 데이터처럼 ID가 없으면 종류별 기본 이미지로 안전하게 되돌아간다.
