...
학습 대시보드는 추천 10개와 선택 과목의 해금된 전체 문제 목록을 분리해 조회한다. 전체 목록은
...
숙련도는 Python·SQL·머신러닝 주제별 전체 과제 중 완료한 과제 비율이다. 해당 과제가 없으면 0%를 표시한다.
...
카드의 `summary`는 `[문제]` 구간에서 제목 표기를 제거한 실제 문제 문장만 사용한다. 상세 화면은 전체
설명을 유지하며 제목을 한 번만 보여 준다. 객관식 상세는 설명 높이를 측정해 글꼴과 선택지 시작 위치를
계산하고, 선택지 버튼도 줄바꿈하므로 긴 이야기·질문·선택지 네 개가 서로 겹치지 않는다.
