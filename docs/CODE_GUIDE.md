...
`CodeChallengeView.editorMode`가 편집 계약을 구분한다. 백엔드 Python 과제의 `program` 모드는 빈 편집기에서 시작해 표준 입력과 출력을 사용하는 전체 프로그램을 제출한다. SQL의 `query` 모드는 짧은 안내 주석만 보여 주고 SQL 문 전체를 제출한다. 로컬 fallback의 `function` 모드만 실제로 검사하는 `sum_to(n)` 선언을 최소 골격으로 제공한다. 어떤 모드도 정답 구현을 초기값에 포함하지 않는다.

백엔드 `hint_text`가 한 줄이면 기존처럼 힌트 하나로 표시한다. 줄바꿈으로 구분된 경우에는 각 비어 있지 않은 줄을 순서가 있는 단계형 힌트로 변환하며, 학습자는 앞 단계부터 차례로 공개한다. 이를 통해 입력 파악·핵심 풀이·출력 확인을 한꺼번에 노출하지 않고 필요한 만큼만 볼 수 있다.

코드 입력은 `StudyModal`이 `CodeEditorOverlayFactory` 계약으로 생성하고, `app/CodeMirrorEditorOverlay.ts`가
코드 과제를 열 때만 CodeMirror 런타임을 불러와 Canvas의 논리 편집 영역에 맞춰 배치한다. 현재 모드의 초기값과 학습자가 편집한 전체 문서가 제출된다. 다른 학습 화면으로 전환할 때만 오버레이를 폐기한다. 코드 채점 결과는 편집기와 겹치지 않는 왼쪽 결과 패널에 표시하므로 편집기 인스턴스, 커서, 스크롤과 실행 취소 기록을 유지한다.

풀이 화면의 왼쪽 패널은 Python에서 `문제 / 공개 예시 / 힌트`, SQL에서 `문제 / 데이터 / 힌트` 탭으로
나뉜다. SQL의 공개 setup SQL은 `services/sqlDataset.ts`가 테이블·열·샘플 행으로 변환하고 행·열 구분선이 있는 표로 표시하며 정답 쿼리는
...
객관식 비율을 따르므로 아직 열지 않은 문제도 코드형으로 잘못 표시되지 않는다. 학습 대시보드는 추천 10개와
...
데일리 퀘스트의 연속 학습 표시는 오늘 문제를 하나 이상 완료한 뒤에만 활성화한다. 완료한 문제가 없으면
출석 행이 생성되어 있더라도 연속 일수를 노출하지 않고 학습 시작 안내를 표시한다.
...
보유 화면의 기본 숲 배경 카드는 `applyRoomTheme(null)`을 호출해 구매한 배경은 유지한 채 적용 상태만 해제한다. 프로필 이미지 변경은 현재 대표 고양이 선택 화면으로 연결되며, 프로필 화면의 도감은 보유 여부와 관계없이 전체 고양이를 보여 준다.

원격 모드의 사운드와 화면 움직임 설정은 서버 스냅샷의 `settings`에 저장한다. 보유 고양이별 기억도
고양이 스냅샷의 `memories`로 읽는다. 전체 기억 초기화는 `DELETE /api/v1/game/cat-memories`, 대화 중인
고양이 한 마리의 기억 초기화는 `DELETE /api/v1/cats/{cat_asset_public_id}/memories`를 사용한다.
...
자유 입력은 Canvas 입력 상자와 `TextInputBridge`를 통해 `chatWithCat()`으로 전달한다. 열린 대화창의 최근 10개 발화도 원문 저장 없이 요청 문맥으로만 함께 전달한다. 서버는 입력을
정규화한 뒤 프롬프트 제어 시도와 지원하지 않는 지식 질문을 생성 모델보다 먼저 차단한다. 사용자 원문은
저장하지 않고, 허용된 코딩·일상 대화만 서버가 만든 분류 요약으로 기억한다. 서버 답변도 길이와 내부 지시
노출 여부를 검사한 뒤 `GameText.text`로 화면에 전달한다. 대화 API의 `memory`가 객체이면 기억이 저장된
것이고 `null`이면 저장되지 않은 것이다. 화면 분기용 대화 유형은 사용자가 입력한 문장을 같은 로컬 가드로
분류하며, 기억 개수는 기억 저장 뒤 다시 읽은 서버 스냅샷을 기준으로 표시한다.

코드 과제의 CodeMirror 편집기는 Canvas 위의 별도 DOM 오버레이다. 처음 표시될 때 최종 화면 크기로 다시
측정하고 첫 줄을 화면 위에 고정한다. `처음으로` 버튼으로 템플릿을 복원할 때도 커서와 스크롤을 첫 줄로
되돌려 코드가 편집기 아래쪽에 가려지지 않게 한다. CodeMirror는 줄 배치와 들여쓰기에 필요한 스타일을
런타임에 생성하므로 Nginx가 요청별 CSP nonce를 HTML 메타 태그와 `style-src`에 함께 주입한다. 편집기는
이 nonce만 동적 스타일에 사용하며 일반 인라인 스타일과 외부 스크립트는 계속 차단한다.
...
대화창의 `기억 초기화`는 복구할 수 없는 작업이므로 확인창을 거친다. 확인하면 현재 대화 중인 고양이의
기억만 삭제하고 다른 고양이 기억과 보유 자산·학습 기록은 유지한다. 성공 후 화면의 기억 개수를 0으로
갱신하고, 실패하면 대화창 안에 재시도 안내를 표시한다.
...
화면 투영은 먼 쪽 가로 폭이 좁고 가까운 쪽 가로 폭이 넓은 사다리꼴 벨트뷰다. `BackgroundGrid`가
`activeWallpaper`에 맞는 바닥 시작선·앞쪽 경계·가로 폭을 선택하며, 고양이와 가구, 포인터 입력은 반드시
같은 프로필을 사용한다. 일반 플레이에서는 격자를 숨기고 꾸미기 모드에서만 배치 셀을 표시한다. 오브젝트
표시 순서는 `x + y`가 아니라 발 또는 바닥 접점의 `y` 깊이를 기준으로 정한다.
...
기본 숲 배경의 잔디 지평선은 기준 화면 `y = 560`이고, 다른 배경은 이미지의 실제 빈 바닥 면에 맞춘
`farY`, `nearY`, 먼/가까운 폭을 `BackgroundGrid`에 등록한다. 가구는 아이소메트릭 시점이 아니라 낮은 횡스크롤 벨트뷰로 제작하며, 러그와 방석처럼 바닥에 놓이는 자산은
...
함수 작성형 과제의 CodeMirror 편집기는 함수 선언과 본문을 포함한 시작 코드 전체를 입력해 둔다. 줄 번호와 포커스 상태를 표시하고,
시작 코드 복원과 클립보드 붙여넣기를 제공한다. 오답이나 일시적인 채점 실패 뒤에는 작성한 전체 코드와 사용한 힌트를
...
`CodeChallengeView.language`은 구문 강조 언어를, `editorMode`는 `program`·`function`·`query`별 안내와
초기값 계약을 결정한다. 제출 시에는 언제나 편집한 전체 문서를 채점기에 전달한다.
...
`StudyModal` 첫 화면은 숲과 나무 프레임 배경 위에 주제별 숙련도·추천 과제와 Python·SQL·머신러닝 카드를 표시한다.
`StudyLandingUi`는 종이 패널, 잎 장식, 주제 이름과 화살표가 있는 카드의 Canvas 표현을 담당한다. 배경에는 글자나 버튼을 넣지 않으며,
첫 화면은 1600×1000 기준으로 숲에 둘러싸인 나무판과 하단 잎 장식을 배치한다. Canvas 크기에 맞춰 배경은 화면을 채우고, 패널·버튼·글자는 같은 기준 좌표를 가로와 세로에 적용해 함께 맞춘다.
샴고양이는 별도 투명 이미지로 나무판 오른쪽 위 테두리에 앞발을 걸친다. `ui.scene.study-forest-board.03`과 `ui.scene.study-siamese-leaning.01`을 앱 조립 계층에서 로드해 `HomeScene`을 통해 전달한다.
제목 아래에는 별도 인사말을 두지 않고 주제 카드별 설명 문구를 표시한다. 과제 목록이나 풀이로 전환할 때는 첫 화면 배경을 숨기고 기존 1600×900 과제 화면 좌표를 사용한다.

숙련도는 서버가 반환한 공식 Concept별 누적 정답 문제 수를 현재 해금된 활성 문제 수로 나눈 값이다. 날짜가
바뀌어도 초기화되지 않으며, 시도가 없는 Concept은 `미측정`으로 표시한다. Concept 필터와 라벨은 문제 응답의
`concept_name`을 기준으로 만들기 때문에 DB에 공식 Concept를 추가해도 화면 목록을 별도로 하드코딩하지 않는다.
현재 티어, 전체 승급 문제 수와 Concept별 승급 필요 수를 함께 표시한다.
추천 과제는 기존처럼 미완료 과제를 우선하고, 전부 완료했으면 첫 과제를 복습한다. 진행률과 페이지 수는 실제 데이터에서 계산한다.
주제 선택은 `studySubjects`를 `subjectPageSize`(3개) 단위로 나누어 한 줄에 표시한다. 양 끝 페이지의 이동 버튼은 비활성화한다.

주제를 선택하면 숙련도·추천 과제를 숨기고 필터와 과제 목록을 표시한다. 과제는 페이지당 최대 6개, 2열×3행이며 7번째부터 다음 페이지다.
풀이에서 돌아올 때는 주제·필터·과제 페이지를 유지하고, 목록에서 뒤로 가면 주제 선택으로 돌아간다. 첫 화면의 뒤로 가기는 홈으로 돌아간다.
학습 화면 상단은 서버 스냅샷의 현재 코인을 표시하며 정답 보상 뒤 새 스냅샷 값으로 즉시 갱신한다. 프로필 화면은
같은 누적 숙련도 계약으로 현재 티어, 누적 정답 문제, 총 풀이 횟수와 현재 숙련도를 보여 준다.

`StudyTaskView.language`는 퀴즈와 코드 과제 모두의 주제를 전달한다. 로컬 문제 은행은 `python`, `sql`, `machine-learning`을 제공하고,
서버 과제는 `domain`의 `PYTHON`, `SQL`, `MACHINE_LEARNING` 값을 같은 화면용 값으로 변환한다. 로컬 문제 은행의 Python과 SQL은
각각 7개 과제로 구성하며 선택형과 코드 작성형을 함께 제공한다. 머신러닝은 기초·응용·도전 난이도의 선택형 과제를 제공한다.
한 페이지에는 6개를 표시하므로 Python과 SQL의 7번째 과제는 `2 / 2` 페이지에서 확인한다.

```json
{ "id": "sql-select-001", "type": "quiz", "language": "sql" }
```
