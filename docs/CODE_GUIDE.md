...
긁기·수면 같은 긴 동작 뒤에는 다음 행동까지 충분히 쉰다. 걷기와 달리기는 목적지에 닿더라도 현재
보폭의 마지막 프레임까지 재생한 뒤 대기로 돌아가며 달리기는 막히지 않은 두 칸 경로를 우선 사용한다.
이동 중 목적지를 다시 지정해도 같은 클립을 처음부터 재생하지 않는다.
...
설치형 PWA는 `catalog.json`, 현재 사용하는 흰 고양이 PNG 시트와 로딩 배경을 service worker에
미리 저장한다. 카탈로그에는 남아 있지만 현재 화면에서 사용하지 않는 검은 고양이 시트는 설치 용량을
늘리지 않도록 precache에서 제외한다.
