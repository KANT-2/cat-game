...
함수 작성형 과제는 UI가 함수 선언과 본문을 포함한 `starterCode`를 초기값으로 보여 준다. 학습자는 전체 코드를 편집하고 `submitCodeChallenge()`로 제출한다. 로컬 클라이언트는 임의 코드를 실행하지 않고 필수 진입점과 허용된 풀이 형태를 테스트 결과로 변환한다. 서버 채점기로 교체할 때도 아래 결과 계약을 유지한다.

코드 입력은 `StudyModal`이 `CodeEditorOverlayFactory` 계약으로 생성하고, `app/CodeMirrorEditorOverlay.ts`가
코드 과제를 열 때만 CodeMirror 런타임을 불러와 Canvas의 논리 편집 영역에 맞춰 배치한다. 함수 선언은 본문과 함께 편집기 초기값에 포함되며 전체 문서가 제출된다. 학습 화면 전환이나 피드백 표시 때 오버레이를 숨기거나 폐기하여 다른 Canvas 입력을
가리지 않도록 한다.
...
`CodeChallengeView.language`은 `python` 또는 `sql`이며 풀이 화면의 편집기 안내를 결정한다. 두 언어 모두
`starterCode`에 전체 답안 초안을 담아 첫 줄부터 자유롭게 편집하고, 제출 시 전체 문서를 채점기에 전달한다.
