...
기본 숲 환경 이미지와 숲 테마 오브젝트는 다음 값을 바꾸지 않는다.
...
새 장소 배경은 `10 × 8` 논리 격자와 벨트뷰를 유지하되 이미지의 실제 빈 바닥 면을 기준으로
`src/game/forest/BackgroundGrid.ts`에 투영 프로필을 등록한다. `farY`는 벽·물·고정 가구가 끝난 뒤,
`nearY`는 난간·책상 턱·전경 장식이 시작되기 전에 둔다. 먼/가까운 폭도 좌우 고정 오브젝트와 겹치지
않게 잡는다. 배경 전용 오브젝트는 해당 프로필의 중앙 깊이에서 먼저 합성 검수한다.

기본 숲 카메라 계약 자체를 변경할 때는 배경만 새로 만들지 않는다. `src/game/config.ts`의
`CLEARING_GRID`, 카메라 가이드 SVG, 이 문서와 모든 숲 환경 오브젝트를 같은 변경에서 갱신한다.
