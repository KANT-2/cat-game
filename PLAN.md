...
프로그래밍 초보 학습자가 짧은 Python 연습 과제를 풀고, 얻은 학습 재화로 고양이를 수집하고 한 화면 숲 공터를 꾸미는 온라인 게임을 만든다.

핵심 반복 구조는 다음과 같다.

```mermaid
flowchart LR
    A[벨트뷰 고양이 숲 공터] --> B[연습 과제 선택]
...
이 계획은 `CONTEXT.md`, `docs/PDR.md`, `docs/adr/*`의 학습·보상·안전 결정을 유지한다. 플랫폼 우선순위는 ADR 0041에 따라 **일반 브라우저 웹 앱**을 우선한다. 게임 화면은 하나의 PixiJS Canvas로 유지하고, 데스크톱 위젯 결합은 게임 코어가 안정된 이후의 선택적 어댑터로 둔다.

- 일반 브라우저용 웹 앱을 하나의 Canvas 코드베이스로 제공한다.
- MVP의 직접 코딩 언어는 Python이다.
- 문제는 검증된 문제 은행에서 제공하고 결정론적 테스트로 채점한다.
- 오답에 벌칙을 주지 않으며 재시도를 제한하지 않는다.
- 하우징은 학습 성능과 무관한 꾸미기 기능이다.
- 숲은 통짜 배경 이미지가 아니라 원경·지면·독립 오브젝트·전경 레이어로 구성한다.
...
1. **메인화면 프로토타입**: 한 화면 숲 공터의 렌더링과 조작을 검증한다.
2. **플레이 가능한 세로 슬라이스**: 공터에서 과제를 풀고 보상을 받아 가구를 놓는 한 바퀴를 완성한다.
...
- **TypeScript + PixiJS**: 숲 공터, HUD, 메뉴, 과제 화면, 모달, 텍스트, 애니메이션과 포인터 판정을 모두 하나의 Canvas에서 담당한다.
- **Vite**: 웹 개발·빌드 환경으로 사용한다.
- **웹 앱 셸**: 해시 정적 자산은 HTTP 장기 캐시로 재사용하고 HTML과 카탈로그는 매 접속 시 갱신한다.
...
다음 구조는 웹 MVP 이후 선택적으로 제공한다. 현재 게임 구현과 웹 출시에 포함하지 않는다.
...
위젯 창 전체가 투명 사각형으로 마우스를 막지 않도록 실제 공터와 버튼 영역만 입력 영역으로 등록한다. 투명 픽셀의 클릭은 아래의 바탕화면으로 전달한다. 편집 모드에서는 전체 위젯 경계를 입력 가능하게 바꾼다.
...
- X11 입력 영역을 공터의 불투명·상호작용 영역에 맞춰 갱신한다.
...
현재 단일 웹 앱의 실제 구조와 역할별 경계는 `docs/ARCHITECTURE.md`를 기준으로 한다. 서버와 샌드박스가 추가되어 단일 앱 유지가 어려워질 때만 다음 워크스페이스 구조로 확장한다.

```text
apps/
  web/                 PixiJS Canvas 앱과 웹 진입점
  api/                 게임 API 서버
workers/
  sandbox-worker/      Python 제출 실행 워커
packages/
  domain/              공통 타입과 게임 규칙
  space-engine/        격자 좌표, 깊이 정렬, 충돌, 이동
  content-schema/      문제와 테스트 데이터 스키마
  canvas-ui/           버튼, 패널, 목록, 텍스트 등 Canvas UI
  platform-bridge/     후속 플랫폼 어댑터용 경계
public/assets/
  forest/              원경과 독립 환경 오브젝트
  furniture/           배치 가구 스프라이트
  cats/                고양이 스프라이트 시트
  ui/                  아이콘과 프레임
```
...
웹 메인화면은 하나의 Canvas 안에서 다음 렌더 레이어로 나눈다.

1. **배경 레이어**: 하늘, 먼 숲과 공터 지면
2. **게임 장면 레이어**: 독립 나무·바위, 가구와 고양이
3. **Canvas UI 레이어**: 프로필, 재화, Study, 퀘스트, 고양이 목록, 설정
4. **전경·오버레이 레이어**: 전경 수풀, 모달, 말풍선, 알림과 화면 전환 효과

HUD도 PixiJS로 그리지만 숲 장면의 스테이지 컨테이너에는 넣지 않는다. 공터는 기준 해상도 전체를 비율에 맞춰 확대·축소하고 HUD는 화면 좌표에 고정한다. 텍스트는 이미지에 굽지 않고 PixiJS `Text` 또는 `BitmapText`로 렌더링한다.

### 4.2 고정 카메라 벨트뷰 격자

공터는 좌우 `x`와 화면 깊이 `y`로 이동하는 한 화면 벨트뷰로 잡는다. 저장과 배치 판정에는 기존 논리 격자를 사용하지만 일반 화면에서는 셀을 숨긴다.

- 논리 격자: `10 × 8`
- 기준 논리 화면: `1600 × 900`, 반응형 축소·확대
- 먼 경계: `y = 392`, 가로 폭 `1240`
- 가까운 경계: `y = 832`, 가로 폭 `1560`
- 좌표 변환 개념:

```text
depth = gridY / gridHeight
stageWidth = lerp(farWidth, nearWidth, depth)
screenX = centerX + (gridX / gridWidth - 0.5) * stageWidth
screenY = lerp(farY, nearY, depth)
```

논리 좌표는 해상도와 분리한다. 화면 크기가 바뀌면 스테이지 컨테이너의 배율과 위치만 조정한다. 캐릭터와 오브젝트는 바닥 접점의 `y`를 기준으로 정렬한다.
...
type ClearingSnapshot = {
  ...
  height: number;
  themeId: string;
  objects: ClearingObject[];
  cats: ClearingCatState[];
};
...
`ClearingSnapshot`은 서버 확장 시 사용할 직렬화 가능한 목표 모델이다. PixiJS의 `CatActor` 같은 렌더 객체를 상태에 넣지 않는다. 현재 프로토타입의 저장 형식은 `GameState`를 기준으로 한다.
...
기본 정렬 키는 바닥 접점의 `y`로 잡고 다음 우선순위를 함께 사용한다.

```text
배경 < 지면 장식 < 가구/고양이 < 전경 장식 < HUD 효과
```
...
숲 전체 그림을 만들지 않고 다음 단위로 제작한다.

| 자산 | 초기 규격 | 비고 |
| --- | --- | --- |
| 숲 원경 | 화면 비율별 레이어 | 하늘과 먼 숲, 텍스트 없음 |
| 환경 오브젝트 | 나무·바위·수풀 | 투명 PNG/WebP, 바닥 접점 필수 |
...
- 기준 해상도, 공터 투영 범위와 논리 격자 크기 확정
- 색상·외곽선·광원·캐릭터 비율을 담은 간단한 아트 가이드
- 클라이언트·서버·샌드박스 경계 확정
- 일반 웹 셸과 해시 정적 자산 HTTP 캐시 기술 스파이크 완료
- 아래의 미결정 사항 중 세로 슬라이스에 필요한 항목 결정

완료 조건:

- 임시 지면과 오브젝트만으로도 좌표·정렬 규칙을 설명할 수 있다.
- 공터 데이터가 특정 이미지 배치에 종속되지 않는다.
- 배포 빌드가 service worker를 등록하지 않고 해시 정적 자산에 장기 캐시 헤더를 제공한다.
...
- 일반 브라우저 웹 셸과 정적 자산 HTTP 캐시
- 온라인 기능의 연결 상태 안내
- 직렬화 가능한 상태에서 생성되는 10×8 벨트뷰 숲 공터
- 원경, 지면, 독립 환경 오브젝트와 전경 레이어
- 서로 다른 크기의 임시 가구 5개
- 배치, 이동, 회전, 회수와 충돌 검사
- 고양이 1마리의 대기와 격자 이동
...
- 통짜 배경 이미지 없이 숲 공터가 구성된다.
- 데스크톱 포인터로 공터와 편집 모드를 사용할 수 있다.
- 오브젝트가 겹칠 때 표시 순서가 안정적이다.
- 화면 비율이 달라져도 공터와 HUD가 잘리지 않는다.
- 일반 브라우저에서 동일한 게임 흐름을 제공한다.
- 새로고침 후 최신 HTML과 카탈로그를 읽고 변경되지 않은 정적 자산은 재사용한다.

### 단계 2 — 플레이 가능한 세로 슬라이스

한 번의 전체 반복 구조만 완성한다.

1. 메인 숲 공터에서 `Study` 선택
2. 문제 은행의 고정 퀴즈 한 문제 제공
3. 정답·오답과 재시도 표시
4. 최초 완료 학습 재화 지급
5. 숲 공터로 복귀해 가구 하나 구매
6. 구매한 가구 배치
7. 앱 재시작 후 진행 상태 복원
...
- 실패·재접속 상황에서도 획득한 가구와 공터 배치가 일관된다.
...
- Chromium 계열 일반 브라우저 모드
- service worker 미등록과 정적 자산 HTTP 캐시 재사용
- 마우스, 터치, 키보드 조작
- 긴 한국어 문구와 큰 재화 숫자
- 느린 네트워크와 저장 실패
- 가구가 많은 공터의 프레임 성능
